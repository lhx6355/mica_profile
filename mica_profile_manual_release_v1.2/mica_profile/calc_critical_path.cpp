				...
				for (int j = 0; j < SREG_SIZE; j++) {
					if (it->isDesLegel(i) && jt->isSrcLegel(j))
						if (jt->SReg[j] == it->DReg[i] && jt->isFpSrcReg(j) == it->isFpDesReg(i)) {
							...
							if (!(flag_it == numIt && flag_jt == numJt))
							{
								(*dg).addEdge(numIt, numJt, DELAY);
							}
							flag_it = numIt;
							flag_jt = numJt;
...
int LongestDepGraph::longestPath(const int& s) {
	...
	while (Stack.empty() == false) {
		...
		if (dist[u] != NINF) {
			...
			for (it = adj[u].begin(); it != adj[u].end(); ++it) {
				//int a = dist[u];
				if (dist[it->getV()] < dist[u] + DELAY)
					dist[it->getV()] = dist[u] + DELAY;
			}
		}
	}
	delete[] visited;
	...
	delete[] dist;
	return criticalPathLength + 1; //[pz]
}

double CriticalPath::cal_KROB()
{
	double krob = 0;
	int num_rob = 0;
	for (int i = 0; i <= robSize; i++)
	{
		krob += (i * criticalPathLength[i]);
		num_rob += criticalPathLength[i];
	}
	krob = krob / num_rob;
	return krob;
}
...
//test
void LongestDepGraph::printadj(LongestDepGraph* dg)
{
	for (int i = 0; i < numV; i++)
	{
		cout << endl << "i=" << i << endl;
		for (deque<AdjListNode> ::iterator kt = (*dg).adj[i].begin(); kt != adj[i].end(); ++kt)
		{
			cout << "v=" << kt->v << "\t" << "weight=" << kt->weight << endl;
		}
	}
}