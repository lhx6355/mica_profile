...
void statisticTrace(InstTrace* instTrace, const int& size,const int& robSize, const char* file_fea_path, const char* file_mica_path, bool nointerval) {
	int *instA64Counters = new int[INST_A64_SIZE]();	//[pz:new]
	int *instA32Counters = new int[INST_A32_SIZE]();
	int *latencyA64 = new int[INST_A64_SIZE]();
	int *latencyA32 = new int[INST_A32_SIZE]();

	/* initialize cycles of instruction latency */
	initLatency(latencyA32, latencyA64);

	/* 7 dimensions */
	CriticalPath critical(robSize);
	...
	delete[] instA64Counters;
	delete[] instA32Counters;
	delete[] latencyA64;
	delete[] latencyA32;
	return;
}
...
void initLatency(int* latencyA32Ptr, int* latencyA64Ptr) {
	...
	for (int i = aarch64::AddXImm; i <= aarch64::Rev3264; ++i)
		latencyA64Ptr[i] = 1;
	for (int i = aarch64::Clz64; i <= aarch64::Cls64; ++i)
		latencyA64Ptr[i] = 2;
	for (int i = aarch64::AndXImm; i <= aarch64::EorXImm; ++i)
		latencyA64Ptr[i] = 1;
	...
	for (int i = aarch64::Smulh64; i <= aarch64::Umulh64; ++i)
		latencyA64Ptr[i] = 6;
	for (int i = aarch64::B64; i <= aarch64::Blr64; ++i)
		latencyA64Ptr[i] = 1;
	for (int i = aarch64::Svc64; i <= aarch64::Eret64; ++i)
		latencyA64Ptr[i] = 2;
	for (int i = aarch64::LDXRB64; i <= aarch64::BigFpMemPreLd; ++i)
		latencyA64Ptr[i] = 4;
	for (int i = aarch64::STXRB64; i <= aarch64::BigFpMemPreSt; ++i)
		latencyA64Ptr[i] = 1;
	for (int i = aarch64::FAbsS; i <= aarch64::FCSelD; ++i)
		latencyA64Ptr[i] = 2;
	for (int i = aarch64::FCmpRegS; i <= aarch64::FmovRegD; ++i)
		latencyA64Ptr[i] = 3;
	for (int i = aarch64::FAddS; i <= aarch64::FMinNMD; ++i)
		latencyA64Ptr[i] = 3;
	for (int i = aarch64::FmovRegCoreW; i <= aarch64::FmovUCoreRegX; ++i)
		latencyA64Ptr[i] = 5;
	for (int i = aarch64::FcvtFpDFpS; i <= aarch64::FRIntID; ++i)
		latencyA64Ptr[i] = 3;
