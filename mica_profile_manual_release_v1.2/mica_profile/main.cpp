...
//example arg: E:\VS2022\projects\mica_profile\instrace.new.log E:\VS2022\projects\mica_profile 100000 instrace.test AARCH64 40

int intervalCount = 1; //当前interval数
int main(int argc, char* argv[])
{
	if (argc == 7)
	{
		...
		if (!in.is_open())
		{
			...
			while (!in.eof())
			{
				...
				std::cout << "running:interval_" << intervalCount << "......" << endl;

				statisticTrace(instTrace, size, robSize, file_fea_path, file_mica_path, nointerval);
		...
		in.close();
		cout << "All intervals completed" << endl;
		...
		//测试手动输入的指令
		InstTrace* instTrace;
		instTrace = new InstTrace[5];

		instTrace[0].pc = 0xefeed7d4;
		instTrace[0].opcode = 0x91400401;
		instTrace[0].inst_paddr = instTrace[0].pc;
		instTrace[0].inst_iset = ISET_AARCH64;

		instTrace[1].pc = 0xefeed7d8;
		instTrace[1].opcode = 0x91400001;
		instTrace[1].inst_paddr = instTrace[1].pc;
		instTrace[1].inst_iset = ISET_AARCH64;

		instTrace[2].pc = 0xefeed7d8;
		instTrace[2].opcode = 0x91000401;
		instTrace[2].inst_paddr = instTrace[2].pc;
		instTrace[2].inst_iset = ISET_AARCH64;

		instTrace[3].pc = 0xefeed7d8;
		instTrace[3].opcode = 0x8a000001;
		instTrace[3].inst_paddr = instTrace[3].pc;
		instTrace[3].inst_iset = ISET_AARCH64;

		instTrace[4].pc = 0xefeed7d8;
		instTrace[4].opcode = 0x8a000401;
		instTrace[4].inst_paddr = instTrace[4].pc;
		instTrace[4].inst_iset = ISET_AARCH64;

		statisticTrace(instTrace, 5, 40, file_fea_path, file_mica_path, nointerval);
	...
	else
	{
		...
		cout << "arg1:input file path" << endl;
		cout << "arg2:output file path" << endl;
		cout << "arg3:interval size" << endl;
		cout << "arg4:output file name" << endl;
		cout << "arg5:AARCH" << endl;
		cout << "arg6:rob size" << endl;
	}
}
